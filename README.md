...
If you wish to adjust these parameters, configuration can optionally be passed via a keyword list. 
  ...
  - `format` (default: `:plaintext`) - What informs separator selection. Because we are trying to preserve meaning between the chunks, the format of the text we are splitting is important. It's important to split newlines in plain text; it's important to split `###` headings in markdown. `TextChunker.supported_formats/0` returns the accepted values, for validating a runtime-provided format before calling `split/2`.
